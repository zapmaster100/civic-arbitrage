...
function render(){
 ...
 <aside><h2><span class="playerdisc p${p.i}"></span>${p.name}</h2><div class="money">Hidden cash: $<b>${p.cash}</b></div><p>Tokens available: ${p.tokens}</p><h3>${setupDraft?'Starting parcel draft':'Action: '+modeLabel()}</h3><p>${setupDraft?'Choose any parcel except Municipal land. Existing private buildings may be acquired; pay $0 during the starting draft.':help()}</p>${mode==='zone'&&selected&&!pendingCouncil?zoneControls():''}${pendingCouncil?councilControls():''}${pendingBuilding!==null?buildingControls():''}${actions<=0&&!setupDraft?'<div class="noactions"><b>No more actions.</b> End your turn.</div>':''}<button id="end" ${setupDraft?'disabled':''}>${actions<=0?'End Turn':'End turn early'}</button><hr><h3>Growth</h3><p>Population ${population} / Jobs ${jobs}</p><p>Thresholds: 4 → 9 → 15</p><hr><h3>Game Log</h3><div class="gamelog">${gameLog.length?gameLog.slice().reverse().map(msg=>'<div>'+msg+'</div>').join(''):'<div>No actions yet.</div>'}</div></aside></main><div id="rulesModal" class="rulesmodal" hidden><div class="rulespanel"><button id="rulesClose" class="rulesclose" type="button">×</button>
<h2>Civic Arbitrage — Playas de México</h2>
<p>Each player is a private developer helping shape one shared growing city — but nobody is building for the public good. Roads, zoning, new buildings and neighbouring development can all increase the value of land owned by somebody else. Your job is to recognize where the city is going, secure valuable parcels, encourage development around your holdings, and turn public growth into private wealth.</p>
<p>There is no visible victory-point score. Cash is hidden. At the end of the game, your hidden cash and the current value of the properties you still own make up your total wealth. The richest developer wins.</p>
<h3>Your Turn</h3>
<p>You normally take <b>2 actions</b>. Actions may be repeated. Choose from the five actions below.</p>

<h3>1. Acquire Parcel</h3>
<p>Choose any unowned parcel except Municipal land. You do not need to own adjacent property. Existing private buildings do not prevent a parcel from being acquired.</p>
<p>You pay the parcel's <b>current land value</b> to the bank and place one of your ownership tokens on it. Greenfield has no zoning value and may therefore be free.</p>
<p><b>Land value:</b> add the parcel's own zoning value to the zoning value of each orthogonal neighbour (north, south, east and west). Low ● is worth $1, Medium ●● is worth $2 and High ●●● is worth $3. Water does not have zoning, but a waterfront edge mirrors the parcel's own zoning value. Buildings themselves add no land value.</p>

<h3>2. Sell Parcel</h3>
<p>Choose one of your properties. The bank pays you its <b>current land value</b>, using the same calculation as Acquire Parcel. Remove your ownership token and return it to your supply. The parcel becomes available for another player to acquire.</p>
<p>Zoning, buildings and roads remain on the board after a sale.</p>

<h3>3. Build Road</h3>
<p>A road action lets you build <b>up to 2 connected road segments</b>. Each segment costs $1. New roads must connect to the existing road network.</p>
<p>Roads are public infrastructure: nobody owns them and there are no tolls. Each road along a parcel edge provides 1 frontage to that parcel. A waterfront edge also provides 1 frontage.</p>

<h3>4. Zone / Rezone</h3>
<p>Choose one of your parcels and propose Residential, Commercial or Municipal zoning at Low ●, Medium ●● or High ●●● density. Rezoning must be approved by Council.</p>
<p><b>How the Council bag is made:</b> each of the four orthogonal neighbours contributes exactly 3 Yes/No cubes, creating a 12-cube bag. Two cubes come from the <b>difference in density</b>: same density = YY, one level apart = YN, and two or more levels apart = NN. The third cube compares <b>use</b>: the same use adds Y and a different use adds N. Greenfield has no use; against Low it contributes YNN, and against Medium or High it contributes NNN. Water is treated as Greenfield for Council.</p>
<p>Draw <b>7 of the 12 cubes</b>. Four or more Yes votes approves the zoning. After a failed draw you may accept the failure, or pay to redraw: $1 for Low, $2 for Medium, or $3 for High. You may continue paying for additional redraws while you can afford them.</p>

<h3>5. Take Building</h3>
<p>Choose a building from the market. Moving past an occupied card costs $1 per card passed; that money is placed on those cards. Any subsidy already sitting on the building you take comes to you.</p>
<p>You may either <b>Place</b> the building or <b>Discard</b> it. Placing it requires a legal parcel and pays the normal $5 building payout plus the card's subsidy. Discarding gives you only the subsidy and the building is not constructed. Cancel before confirming to leave the market unchanged.</p>
<p><b>Placement:</b> the parcel must have the correct Residential or Commercial use and zoning density at least as high as the building. Required frontage is Residential 2 / 2 / 3 and Commercial 2 / 3 / 4 for Low / Medium / High.</p>
<p>Low buildings have no neighbourhood prerequisite. A Medium building needs a built Low building of the opposite private use within the surrounding 8 parcels. A High building needs a built Medium building of the opposite private use within those 8 parcels.</p>
<p>A building may be redeveloped only upward within the same use: for example, a Low Residential building may become Medium Residential. The old building is replaced and provides no refund.</p>

<h3>Growth & Municipal Development</h3>
<p>Residential construction increases Population and Commercial construction increases Jobs by the building's density: Low +1, Medium +2, High +3. Growth thresholds are 4, 9 and 15. Municipal development is tied to the city reaching these thresholds.</p>
<p><i>This is a prototype. Growth restrictions and Municipal staging are still being implemented in the digital version.</i></p>
</div></div>;
...
function zoneControls(){const x=parcels.find(q=>q.id===selected);return `<div class="zonecontrols"><p><b>Proposed zoning — Council bag preview</b></p>${['residential','commercial','municipal'].map(z=>`<div><b>${z}</b> ${[1,2,3].map(d=>{const yes=councilOdds(x,z,d);return `<button data-zone="${z}" data-density="${d}">${'●'.repeat(d)} · ${yes}Y/${12-yes}N</button>`}).join('')}</div>`).join('')}</div>`}
