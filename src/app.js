...
function spendAction(){actions--;if(actions<=0)setTimeout(endTurn,250)}
function setMode(m){mode=m;selected=null;render()}
function render(){
 const p=players[turn];
 document.querySelector('#app').innerHTML=`
 <header><div><h1>Civic Arbitrage</h1><p>Playas de México · v0.1 prototype</p></div><div class="turn"><b>${p.name}</b> · <strong>${actions}/2 actions</strong></div></header>
 ...
 <main><section>
 <div class="board">${parcels.map(x=>`<button class="parcel ${x.zone} ${x.water?'water':''} ${selected===x.id?'selected':''}" data-id="${x.id}"><span>${x.id}</span>${x.r===plaza.r&&x.c===plaza.c?'<strong>PLAZA</strong>':''}${x.owner!==null?'<i>'+players[x.owner].name[0]+'</i>':''}${x.density?'<em>'+('●'.repeat(x.density))+'</em>':''}<small>$${value(x)}</small></button>`).join('')}</div>
 ...
 <aside><h2>${p.name}</h2><div class="money">Hidden cash: $<b>${p.cash}</b></div><p>Tokens available: ${p.tokens}</p><h3>Action: ${modeLabel()}</h3><p>${help()}</p>${mode==='zone'&&selected?zoneControls():''}<button id="end">End turn early</button><hr><h3>Growth</h3><p>Population ${population} / Jobs ${jobs}</p><p>Thresholds: 4 → 9 → 15</p></aside></main>`;
...
function parcelClick(id){if(actions<=0)return;const x=parcels.find(q=>q.id===id),p=players[turn];if(x.r===plaza.r&&x.c===plaza.c)return;
