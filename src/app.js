...
let turn=0,actions=CONFIG.actions,mode='acquire',selected=null,population=0,jobs=0,setupDraft=true,draftPick=0;
...
function roadEdges(){let h='';for(let r=0;r<CONFIG.height;r++)for(let c=0;c<CONFIG.width;c++){for(const side of ['N','W',...(r===CONFIG.height-1?['S']:[]),...(c===CONFIG.width-1?['E']:[])]){const key=edgeKey(r,c,side);h+=`<button class="roadedge ${side} ${roads.has(key)?'built':''}" data-edge="${key}" style="--r:${r};--c:${c}" title="${roads.has(key)?'Road':'Build road'}"></button>`}}return h}
function render(){
...
function parcelClick(id){const x=parcels.find(q=>q.id===id);if(x.municipal)return alert('Municipal parcels cannot be privately owned.');
 if(setupDraft){const pi=draftOrder[draftPick],p=players[pi];if(Number.isInteger(x.owner))return alert('That parcel is already owned by a player.');snap();x.owner=pi;p.tokens--;p.owned.push(id);draftPick++;if(draftPick>=draftOrder.length){setupDraft=false;turn=0;actions=CONFIG.actions;}render();return}
