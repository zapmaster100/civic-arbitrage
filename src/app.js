...
<h3>3. Zone / Rezone</h3>
<p>Choose one of your parcels and propose Residential, Commercial or Municipal zoning at Low ●, Medium ●● or High ●●● density. Municipal zoning is always available.</p>
...
function render(){
 ...
 <div class="market"><h2>Building Market</h2>${[0,1].map(row=>`<div class="marketrow">${market.slice(row*5,row*5+5).map((c,j)=>c?`<button class="card" data-card="${row*5+j}"><b>${c.name}</b><span>${c.use}</span><strong>${'●'.repeat(c.density)}</strong><small>Subsidy ${c.coins} · Skip ${j}</small></button>`:`<div class="card card-empty" aria-label="Empty market slot"></div>`).join('')}</div>`).join('')}<div class="municipal-staging">Municipal staging — ${municipalQueue.length?municipalQueue.map(c=>c.name+' '+('●'.repeat(c.density))).join(' · '):'empty'}</div></div></section>
 <aside>${gameOver?'<h2>GAME OVER</h2>':''}<h2><span class="playerdisc p${p.i}"></span>${p.name}</h2><div class="money">Hidden cash: $<b>${p.cash}</b></div><p>Tokens available: ${p.tokens}</p><h3>${setupDraft?'Starting parcel draft':'Action: '+modeLabel()}</h3><p>${setupDraft?'Choose any parcel except Municipal land. Existing private buildings may be acquired; pay its current land value during the starting draft.':help()}</p>${mode==='zone'&&selected&&!pendingCouncil?zoneControls():''}${pendingCouncil?councilControls():''}${pendingBuilding!==null?buildingControls():''}${actions<=0&&!setupDraft?'<div class="noactions"><b>No more actions.</b> End your turn.</div>':''}<button id="end" ${setupDraft?'disabled':''}>${actions<=0?'End Turn':'End turn early'}</button><hr><h3>Growth</h3><p>Population ${population} / Jobs ${jobs}</p><p>Thresholds: 4 → 9 → 15</p><p>${currentGrowthThreshold()?'Current gate: '+currentGrowthThreshold()+' / '+currentGrowthThreshold():(gameOver?'Final Municipal completed':'All growth thresholds reached')}</p><hr><h3>Game Log</h3><div class="gamelog">${gameLog.length?gameLog.slice().reverse().map(msg=>'<div>'+msg+'</div>').join(''):'<div>No actions yet.</div>'}</div></aside></main><div id="rulesModal" class="rulesmodal" hidden><div class="rulespanel"><button id="rulesClose" class="rulesclose" type="button">×</button>${rulesContent()}</div></div>`;
