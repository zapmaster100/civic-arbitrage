...
function render(){
 ...
 <aside><h2><span class="playerdisc p${p.i}"></span>${p.name}</h2><div class="money">Hidden cash: $<b>${p.cash}</b></div><p>Tokens available: ${p.tokens}</p><h3>${setupDraft?'Starting parcel draft':'Action: '+modeLabel()}</h3><p>${setupDraft?'Choose any parcel except Municipal land. Existing private buildings may be acquired; pay $0 during the starting draft.':help()}</p>${mode==='zone'&&selected&&!pendingCouncil?zoneControls():''}${pendingCouncil?councilControls():''}${pendingBuilding!==null?buildingControls():''}${actions<=0&&!setupDraft?'<div class="noactions"><b>No more actions.</b> End your turn.</div>':''}<button id="end" ${setupDraft?'disabled':''}>${actions<=0?'End Turn':'End turn early'}</button><hr><h3>Growth</h3><p>Population ${population} / Jobs ${jobs}</p><p>Thresholds: 4 → 9 → 15</p><hr><h3>Game Log</h3><div class="gamelog">${gameLog.length?gameLog.slice().reverse().map(msg=>'<div>'+msg+'</div>').join(''):'<div>No actions yet.</div>'}</div></aside></main><div id="rulesModal" class="rulesmodal" hidden><div class="rulespanel"><button id="rulesClose" class="rulesclose" type="button">×</button><h2>Civic Arbitrage — Rules</h2><p><b>Goal:</b> Finish with the most wealth: hidden cash plus the current value of property you still own.</p><h3>Turn</h3><p>Take 2 actions. You may Acquire, Sell, Build Road, Zone / Rezone, or Take Building.</p><h3>Land & roads</h3><p>Acquire an unowned non-Municipal parcel by paying its current land value. Sell to the bank for current land value. A road action builds up to 2 connected segments at $1 each.</p><h3>Zoning & value</h3><p>Low ● = $1, Medium ●● = $2, High ●●● = $3. Land value is its own zoning value plus orthogonal neighbouring zoning values. Waterfront mirrors the parcel's own zoning value.</p><h3>Council</h3><p>Rezoning uses 12 cubes generated by the four orthogonal neighbours. Draw 7; 4 Yes passes. After a failed draw you may accept the failure or pay $1 / $2 / $3 for Low / Medium / High to redraw.</p><h3>Buildings</h3><p>Correct use and sufficient zoning density are required. Frontage: Residential 2 / 2 / 3; Commercial 2 / 3 / 4 for Low / Medium / High. Medium and High buildings also require a built opposite-use building one density lower within the surrounding 8 parcels. A placed private building pays $5 plus its market subsidy.</p><h3>Market</h3><p>Taking a card costs $1 on each occupied card passed. You may place it legally for its building payout or discard it only to collect its subsidy. The market shifts and refills at end of turn.</p><p><i>Prototype rules — additional Municipal and growth rules are still being implemented.</i></p></div></div>`;
